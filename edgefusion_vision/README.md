...
全志 **V821 (AvaotaF1)** 上的 RTSP 推流 sample：MIPI CSI 采集 → ISP → **H.264 硬件编码** → 内置 Wi-Fi **RTSP 推流**，供 IMX6ULL 边侧网关拉流。基于全志 eyesee-mpp 多媒体框架的 **`sample_rtsp`**（纯推流，无录像/检测），**脱离 Tina SDK 单独外编**。
...
| 编译产物 | `output/sample_rtsp` + `output/sample_rtsp_strip` |
| 推流地址 | `rtsp://<板子IP>:8554/ch0`（H.264 1920×1080@15fps，2Mbps） |

产物校验（`readelf`）：
- 解释器：`/lib/ld-musl-riscv32.so.1`（musl）
- 文件类型：ELF 32-bit LSB, UCB RISC-V
- NEEDED 动态库：仅 `liblog.so / libasound.so.2 / libstdc++.so.6 / libgcc_s.so.1 / libc.so`（MPP/ISP/RTSP 全静态链接）
- RPATH：`/usr/lib`
...
```
edgefusion_vision/
├── toolchain/              # musl 工具链（已随工程自包含，无需外部 Tina SDK）
├── aw_pack_src/lib_aw/
│   ├── lib/eyesee-mpp/      # 全部 MPP 静态库 .a
│   └── include/             # 头文件（eyesee-mpp/libisp/libcedarc/uapi/...）
├── share_lib/               # 运行期动态库（liblog.so/libasound.so.2 + 传递依赖）
├── sample/
│   ├── sample_rtsp/          # 主 sample 源码（sample_rtsp.c/.h/_config.h，SDK 原版）
│   ├── common/               # 共用源（rtsp_server.cpp / sample_common_venc.c，SDK 原版）
│   └── configfileparser/     # confparser.h
├── output/                  # 编译产物 + 中间 .o + 运行配置
│   ├── sample_rtsp
│   ├── sample_rtsp_strip
│   └── sample_rtsp.conf     # 调好的运行配置（wlan0/1080p/不录像/子码流关）
...
> **本工程已自包含**：musl 工具链 + MPP 静态库/头 + 内核 UAPI 头均已打包进工程（`toolchain/`、`aw_pack_src/`），**无需安装 Tina SDK**。获取整个文件夹即可编译。

### 3.2 工具链与 MPP 库（已随工程自包含）

本工程把以下三样从一次构建好的 Tina SDK 中提取并打包进工程目录：

1. **musl 工具链**：`toolchain/`（约 1.1G，已自包含，不再是指向 SDK 的符号链接）
   - 前缀 `riscv32-linux-musl-gcc/g++`（musl libc，**不是 glibc**）
   - 工具链天生可重定位（`gcc -print-sysroot` 用 `bin/../sysroot` 相对路径），整个文件夹可随意移动
   - 注意：目录名 `nds32le` 是历史遗留，二进制实际是 RISC-V
2. **MPP 静态库 + 头文件**：`aw_pack_src/lib_aw/`（eyesee-mpp/libisp/libcedarc/uapi 全套）
3. **内核 UAPI 头**：`aw_pack_src/lib_aw/include/uapi`

> 仅当 SDK 重新 `make && pack` 后需要更新 bundle 时，才用到完整 Tina SDK（见 §9 更新方法）。
...
```bash
cd ~/EdgeFusion_Smart_NVR/edgefusion_vision
./build.sh
```

`build.sh` 内部执行 `make build_app`，成功后 `output/` 下生成：
- `sample_rtsp`（带 debug 符号）
- `sample_rtsp_strip`（strip 后，部署用这个）

**编译的源文件**（仅 3 个，与 SDK `tina.mk` 的 `sample_rtsp` SRCCS 一致）：
```
sample_rtsp/sample_rtsp.c        # 主程序：采集→编码→RTSP 推流
common/rtsp_server.cpp           # RTSP server（基于 libTinyServer）
common/sample_common_venc.c      # VENC 通道参数辅助
```
...
### 编译注意
- 工具链目录 `toolchain/` 必须存在且完整（已随工程自包含；若缺失需从 Tina SDK 重新拷贝，见 §9）。
...
- `common/rtsp_server.cpp` 与 `.h` 用 **SDK 原版**（未修改），与 SDK 逐字节一致。

---

## 5. 配置

运行配置是 iniparser 风格的 `sample_rtsp.conf`。本工程 `output/sample_rtsp.conf` 是调好的（V821 适配）。关键项：
...
main_src_height = 720
main_encode_width = 1920       # 编码输出分辨率（1080p）
main_encode_height = 1080
main_encode_frame_rate = 15    # 帧率
main_encode_bitrate = 2097152  # 2 Mbps
main_file_path = #"/mnt/extsd/mainStream.raw"   # #注释掉 = 不录像（仅推流）
main_online_en = 1             # 在线编码

# 子码流 + 子延时码流：已禁用（网关只拉主码流 ch0，关掉省 VE/ION）
sub_viChn = -1                 # -1 禁用子码流
sub_venc_chn = -1
sub_lapse_viChn = -1           # -1 禁用子延时码流
sub_lapse_venc_chn = -1
```

> **sample_rtsp 不录像**：`main_file_path` 默认就是 `#` 注释掉（空路径），只推流不写盘。录像由边侧 IMX6ULL 网关负责。

> **sample_rtsp 没有 `main_rtsp_id` 开关**：RTSP 推流在编译期由 `SUPPORT_RTSP_TEST` 宏控制（默认开启），只要 sample 启动就推流到 `rtsp://<IP>:8554/ch0`。
...
# 2. 杀掉旧进程
sudo bash -c 'printf "killall sample_rtsp 2>/dev/null\r\n" > /dev/ttyUSB0'

# 3. 从串口启动（后台 + 重定向日志到 /tmp，避免写 TF 卡）
sudo bash -c 'printf "cd /mnt/extsd && ./sample_rtsp -path /mnt/extsd/sample_rtsp.conf > /tmp/run.log 2>&1 &\r\n" > /dev/ttyUSB0'
```
...
```bash
# 进程 + 端口（adb 查）
adb -s 4c48410491c30a72512 shell 'ps | grep sample_rtsp; netstat -tln | grep 8554'

# 当前 wlan0 IP（DHCP 动态，每次上电可能变）→ 推流地址即 rtsp://<IP>:8554/ch0
adb -s 4c48410491c30a72512 shell 'ip -4 addr show wlan0 | grep inet'
```

> **sample_rtsp（SDK 原版）不打印推流地址**。要知道 URL，用上面的 `ip addr` 查 wlan0 IP，拼成 `rtsp://<IP>:8554/ch0`，并 `netstat` 确认 8554 在听。
...
```bash
sudo bash -c 'printf "killall sample_rtsp\r\n" > /dev/ttyUSB0'
# 或 adb（若进程还在）
adb -s 4c48410491c30a72512 shell 'killall sample_rtsp'
```
...
| 日志报 `vencInit fail`/`ion alloc fail`/段错误 | 反复启动/崩溃泄漏 ION 内存，编码器起不来 | `reboot` 清干净后单次启动；`killall` 后不要立即重启同一程序 |
| 网关拉流报 `not enough frames to estimate rate` / `Could not find codec parameters` | sample_rtsp 在线编码首个 IDR 来得晚，网关探测窗口太紧 | 放宽网关 `stream_receiver.c` 的 `analyzeduration`/`probesize`（已改 5s/10MB） |
| 网关拉流报 `DESCRIBE failed: 404` | V821 sample_rtsp 没在跑，或网关 `rtsp_url` 路径/IP 不对 | 确认 sample_rtsp 在跑、8554 在听；核对网关 `rtsp_url` 是 `rtsp://<V821-IP>:8554/ch0` |
| 日志报 `rtsp_net_type` 网卡找不到 | wlan0 未连上 | 确认 Wi-Fi 已连，`ip addr show wlan0` 有 IP |
| 网关拉不到流 | V821 IP 变了（wlan0 DHCP 动态） | 重新查 V821 IP，更新网关 `gateway.conf` 的 `rtsp_url`（Web 设置页可热改） |
...
1. **库**：用 `out/v821/avaota_f1/openwrt/staging_dir/target/usr/lib/*.a`、`.../usr/lib/eyesee-mpp/*.a` 覆盖 `aw_pack_src/lib_aw/lib/eyesee-mpp/`
2. **头**：用 staging 的 `usr/include/{eyesee-mpp,libisp,libcedarc,*.h}` 及 `out/v821/kernel/build/user_headers/include` 覆盖 `aw_pack_src/lib_aw/include/` 对应目录
