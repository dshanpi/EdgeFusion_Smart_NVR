...
## 6. 部署到板子
由于板端存储空间较小，请提前TF卡插入板子。板子会自动挂载至/mnt/extsd目录下。使用adb将编译的应用程序push到v821板端的/mnt/extsd目录。
```bash
...
在运行程序前需要先执行WiFi配网，使用wifi命令`wifi -c <WIFI名> <WIFI密码>`
...
如果是使用Mobaxterm串口工具登录，可以选择串口，波特率为115200，进入/mnt/extsd/目录下，执行程序：
```bash
cd /mnt/extsd

./sample_smartIPC_demo -path /mnt/extsd/sample_smartIPC_demo.conf
```
