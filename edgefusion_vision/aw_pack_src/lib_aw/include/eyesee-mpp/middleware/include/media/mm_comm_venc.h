...
typedef struct VENC_ATTR_MJPEG_S
{
    ...
    unsigned int  mThreshSize;  /*threshSize in stream buffer, 0 means auto config*/
    /**
      jpeg online encode mode, insert encoding jpeg between encoding frames on other h264 encode channel which is
      also running on online encode mode.
      TRUE: enable, match AW_MPI_VENC_EnableOnlineTakePicture().
      FALSE: online encode mode, and encoding jpeg commonly.
    */
    BOOL bOnlineEnableInsertPicture;
}VENC_ATTR_MJPEG_S;
...
typedef enum VENC_REF_FRAME_LBC_MODE_E
{
    ...
    VENC_REF_FRAME_LBC_MODE_1_0X     = 5,
    VENC_REF_FRAME_LBC_MODE_1_1X     = 6,
    VENC_REF_FRAME_LBC_MODE_1_2X     = 7,
    VENC_REF_FRAME_LBC_MODE_1_3X     = 8,
    VENC_REF_FRAME_LBC_MODE_1_4X     = 9,
    VENC_REF_FRAME_LBC_MODE_1_6X     = 10,
    VENC_REF_FRAME_LBC_MODE_1_7X     = 11,
    VENC_REF_FRAME_LBC_MODE_1_8X     = 12,
    VENC_REF_FRAME_LBC_MODE_1_9X     = 13
...
typedef struct VENC_ATTR_S
{
    ...
    unsigned int mVeRxInputBufmultiplexEnable; /* tdm buff and bk buff shareing, 0:not share(by default), 1:share.*/
    unsigned int mVbrOptEnable;  /* 0: use old vbr bitrate control, 1: use new vbr bitrate control. */
    int mVippID; //only for online. vipp support tdm, VippID here is soft vipp after tdm of hardwareVipp.
    unsigned int mMultiSensorOnlineEnable; /* only for online. multi sensor online scene. */
    unsigned int mImageStitchingEnable; /* only for multi sensor online scene. image stitching. */
    VencSetVbvBufInfo mVbvBufInfo; /* for jpg encoder use yuvFrame as vbvBuffer, yuvFrame is out vbv in this case. */
}VENC_ATTR_S;
...
{
    BOOL bEnableIsp2Ve;
    BOOL bEnableVe2Isp;  //only main venc stream can enable ve2Isp to isp!
    VI_DEV nVipp; //indicate frame source.
    int nVirtualSource; //only for V821 SOFTWARE_TDM_FOR_ISP, vipp1 and vipp5. 0:sensorB, 1:sensorC.
}VENC_IspVeLinkAttr;
