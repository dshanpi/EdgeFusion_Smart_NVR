...
#define HVIDEO(group, index) (group*VIPP_TIMESLOTS + index)
...
typedef struct VI_PTN_CFG_S
{
    FILE *ptn_file; /* pattern mode raw file */
    unsigned char ptn_en; /* pattern mode enable */
...
typedef struct awVI_ATTR_S {
    ...
    unsigned char tdm_tx_valid_num; /* when enable tdm speed down must to set */
    unsigned char tdm_tx_invalid_num;   /* when enable tdm speed down must to set */
    unsigned char tdm_tx_valid_num_offset; /* when enable tdm speed down must to set */
    VI_CROP_CFG_S mCropCfg; /* config vipp crop params.*/
    //BOOL mbEncppEnable; /* deprecated. enable Encpp sharp, FALSE:disable, TRUE:enable, default value:FALSE. if mpp venc enable Encpp sharp, set TRUE.*/
    BOOL mbWidthStrideDisable; /* disable width stride(default 0, means enable).*/
    unsigned int tdm_rxbuf_cnt;
    unsigned char large_dma_merge_en; /* vi_dma_stitch_mode_t, 0: dma not to merge, 1: 2in1 switch mode, 2: dma horizontal merge , 3: dma vertical merge */
    ...
    struct VI_PTN_CFG_S mPtnCfg; /* offline simulate raw config for pattern mode */

    /**
      to support v821 three cameras, two sensors share one vipp, virtual channels will be divided to two halves, one half
      outputs one sensor's frames. e.g., virvi 0 and 1 output one sensor's frames, virvi 2 and 3 output another sensor's
      frames.
    */
    BOOL bSensorSwitch;
    BOOL mbViAndVeVbvShareBufEnable; /* only for jpeg take picture */
} VI_ATTR_S;
...
typedef struct awVI_BUFFER_INFO_S {
	void *phy_addr;
	void *vir_addr;
	int buf_size;
	int offset;
