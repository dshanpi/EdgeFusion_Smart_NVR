...
typedef struct bufMrgQ_t
{
    ...
    struct list_head mHoldFrameList; //used for out_vbv of jpeg encoder.
...
typedef struct VIDEOENCDATATYPE {
    ...
    VencCyclicIntraRefresh mEncIntraRefreshParam;
    ...
    */
    ...
    BOOL mbCedarvVideoEncInitFlag;//the flag of init  video encoder.
    ...
    */
    ...
    int mvipp_id; //get from pEncFrame->VFrame.mWhoSetFlag, which is set by mpi_vi.
...
//    unsigned int mLastSeiDataConfigLevel;
    ...
    int nSetSEICount; //frame count in sei enable.
    int64_t nCheckSeiShellSetTm; //unit:ms
    BOOL bNeedSetSeiFlag;
    ...
    unsigned int mVbvBufThreshSize;
} VIDEOENCDATATYPE;
