...
#ifndef WMG_DATE
#define WMG_DATE " 20240105-10:00"
#endif

#ifndef WMG_BASE_VERSION_NUMBER
#define WMG_BASE_VERSION_NUMBER "2.0.8.5"
#endif

#ifndef WMG_VERSION
#define WMG_VERSION  WMG_BASE_VERSION_NUMBER WMG_DATE WMG_HASH WMG_DIRTY
#endif
...
#ifndef MODULE_NAME_MAX_LEN
#define MODULE_NAME_MAX_LEN 64
#endif

/* What is defined here is the maximum length of the command line parameters entered in the terminal */
...
typedef enum {
	...
	WIFI_SEC_UNKNOWN = 0b1000000000000000,
} wifi_secure_t;
 ...
 * @scan_action: whether to perform a real scan (Currently only valid for system freertos)
 */
typedef struct {
	uint8_t bssid[6];
	char ssid[SSID_MAX_LEN + 1];
 ...
 * @max_num_sta: sta maximum number of connections
 * @dev_list: device list of stations connected to this ap(The application layer needs to apply for memory)
 * @sta_dev_list_num: dev list buff size
...
typedef struct {
	...
	uint8_t ip_addr[4];
	uint8_t gw_addr[4];
	wifi_sta_dev_member_t *dev_list;
	uint8_t sta_dev_list_num;
	uint8_t sta_num;
} wifi_ap_config_t;

/**
 * wifi_monitor_config_t - store wifi monitor configuration
 *
 * @channel: channel
 * @printf_frame: how many frames to print at a time
 */
typedef struct {
	uint8_t channel;
	uint16_t printf_frame;
 ...
 *
 * @dev_name: dev name
 * @discover_time: discover p2p dev time
 * @listen_time: listen p2p dev time(only valid when discover_time is 0)
...
/**
 * wifi_p2p_info_t - store wifi p2p information
 *
 * @bssid: BSSID
 * @mode: go/gc(0:non 1:go 2:gc)
 * @freq: frequency of the channel in MHz (e.g., 2412 = channel 1)
 * @ssid: SSID
 * @ip_addr: ip address
 */
 ...
 */
typedef enum {
	/* The enable state is also the idle state */
	WIFI_P2P_ENABLE,
	WIFI_P2P_DISABLE,
	WIFI_P2P_DISCOVER,
...
typedef enum {
	WMG_LINKD_MODE_BLE,
	WMG_LINKD_MODE_JL6328,
	WMG_LINKD_MODE_SOFTAP,
	WMG_LINKD_MODE_XCONFIG,
	WMG_LINKD_MODE_SOUNDWAVE,
	WMG_LINKD_MODE_QRCODE,
	WMG_LINKD_MODE_ACK,
	WMG_LINKD_MODE_BLE_ACK,
	WMG_LINKD_MODE_JL6328_ACK,
	WMG_LINKD_MODE_SOFTAP_ACK,
	WMG_LINKD_MODE_XCONFIG_ACK,
	WMG_LINKD_MODE_SOUNDWAVE_ACK,
	WMG_LINKD_MODE_QRCODE_ACK,
	WMG_LINKD_MODE_NONE,
} wifi_linkd_mode_t;
...
/**
 * wifi_on - wifi manager open with specified wifi mode
 *
...
/**
 * wifi_off - wifi manager close with specified wifi mode
 *
 * @mode: specified wifi mode, if null will close all mode
 * return: 0 - wifi manager deinit success, others - fail
 */
wmg_status_t wifi_off(wifi_mode_t mode);
 ...
 */
wmg_status_t wifi_monitor_enable(wifi_monitor_config_t *monitor_config);
...
/**
 * wifi_p2p_find - find p2p device (Discover)
 *
 * @find_second: > 0 : how many seconds to find p2p device
 *               = 0 : stop find p2p device
 *               < 0 : the background keeps finding until the connection is successful
 *                     or you enter 0 to stop finding
 * @types: social : Scan only the standard social channels for Wi-Fi P2P (1, 6, 11 in 2.4GHz)
 *       : progressive : Scan the social channel first, and if no device is found, scan other channels
 *       : full : Scans all available 2.4GHz and 5GHz channels
 *       : listen : only listen (allwinner defined, non-standard)
 *
 * return: 0 - find device success, others - fail
 */
wmg_status_t wifi_p2p_find(int find_second, char *types);
 ...
 *
 * @p2p_num: how many p2p device find
 *
 * @p2p_arr_size: how much memory is requested
 *
 * return: 0 - find device success, others - fail
 */
wmg_status_t wifi_p2p_get_peers(wifi_p2p_peers_t *p2p_peers, uint32_t *p2p_num, uint32_t p2p_arr_size);
 ...
 */
wmg_status_t wifi_get_scan_results(wifi_scan_result_t *result, const char *ssid, uint32_t *bss_num, uint32_t arr_size);
 ...
 *
 * @send_cmd: send cmd
 * @expand_params: parameters attached to the command, if not filled in NULL
 * @cb: cmd return valid
 * return: 0 - send cmd success, others -fail
 */
wmg_status_t wifi_send_expand_cmd(char *expand_cmd, void *expand_params, void *expand_cb);
