...
#define RTSP_SERVER_CHANNEL_NUM  (4)
...
int rtsp_open(int id, RtspServerAttr *rtsp_attr)
{
    ...
    if (RTSP_VIDEO_TYPE_H264 == rtsp_attr->video_type) {
        ...
        attr.videoType  = MediaStream::MediaStreamAttr::VIDEO_TYPE_INVALID;
    }

    if (RTSP_AUDIO_TYPE_AAC == rtsp_attr->audio_type) {
        attr.audioType = MediaStream::MediaStreamAttr::AUDIO_TYPE_AAC;
    } else {
        attr.audioType = MediaStream::MediaStreamAttr::AUDIO_TYPE_INVALID;
    }
    ...
    if (RTSP_STREAM_TYPE_UNICAST == rtsp_attr->stream_type) {
        ...
        attr.streamType = MediaStream::MediaStreamAttr::STREAM_TYPE_INVALID;
    }

    /**
     * videoType (0:h265,1:h264,2:invalid)
     * audioType (0:aac,1:invalid)
     * stream_type (0:unicast,1:multicast,2:invalid)
     */
    printf("%s rtsp[%d] videoType:%d, audioType:%d, stream_type:%d\n", net_name, id, attr.videoType, attr.audioType, rtsp_attr->stream_type);
    ...
    stream->setVideoFrameRate(rtsp_attr->frame_rate);
...
void rtsp_close(int id)
{
    ...
    if (gpRtspStream[id])
    {
        delete gpRtspStream[id];
        gpRtspStream[id] = NULL;
    }
    if (gpRtspContext[id])
    {
        delete gpRtspContext[id];
        gpRtspContext[id] = NULL;
    }
}
...
void rtsp_sendData(int id, RtspSendDataParam *param)
{
    ...
    if (stream != NULL)
    {
        if (0 < param->frame_rate && stream->getVideoFrameRate() != param->frame_rate)
        {
            printf("%s,%d: rtsp id %d, framerate change %d to %d\n", __func__,__LINE__, id, stream->getVideoFrameRate(), param->frame_rate);
            stream->setVideoFrameRate(param->frame_rate);
        }
